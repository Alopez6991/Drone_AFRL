...
class MpcDrone:
    def __init__(self,vx,vy,mu,wx,wy,wz,params,psi=None,x=None,y=None,vmu=None,phi=None,theta=None,phidot=None,thetadot=None,psidot=None,x0=None,U_start=None,X_start=None,dt=0.01, n_horizon=20, r_weight=0.0, run=True):
        ...
        self.vmu = np.array([mu[0]]) if vmu is not None else np.array([0.0])
        ...
        self.x_start = X_start if X_start is not None else np.array([self.x[0],self.vx[0],self.y[0],self.vy[0],self.mu[0],self.vmu[0],self.phi[0],self.phidot[0],self.theta[0],self.thetadot[0],self.psi[0],self.psidot[0],self.wx[0],self.wy[0],self.wz[0]])
        ...
        self.uwix=0.0 # wind control
        self.uwiy=0.0 # wind control
        self.uwiz=0.0 # wind control
        ...
        mu = self.model.set_variable(var_type='_x', var_name='mu')
        vmu = self.model.set_variable(var_type='_x', var_name='vmu')
        ...
        mu_setpoint     = self.model.set_variable(var_type='_tvp', var_name='mu_setpoint')
        ...
        # dvzdt dynamics
        dvzdt = (cos(phi)*cos(theta))*U1/m - Dl*vrz/m - g
        ...
        self.model.set_rhs('mu', vmu)
        self.model.set_rhs('vmu', -1*(dvzdt + 2*z*vz*vmu)/ z**2)
    ...
    def set_objective(self, r_weight=0.0): #r_weight=penalize control inputs
        ...
        lterm = (self.model.x['vx']  - self.model.tvp['vx_setpoint'])**2 + \
                ...
                (self.model.x['mu']   - self.model.tvp['mu_setpoint'])**2 + \
    ...
    def mpc_tvp_function(self, t): # important
        ...
        for n in range(self.n_horizon + 1):
            ...
            self.mpc_tvp_template['_tvp', n, 'mu_setpoint']   = self.mu[k_set]
    ...
    def simulator_tvp_function(self, t): # might do nothing
        ...
        self.simulator_tvp_template['mu_setpoint']    = self.mu[k_step]
    ...
    def run_just_simulator(self, Xo=None, Tsim=None, Usim=None, M=None):
        ...
        measurements[:, :15] = self.xsim[:, :15]  # Px, Vx, Py, Vy, Pmu, Vmu, R, dR, P, dP, Yaw, dYaw, Wx, Wy, Wz
        
        # Handle division-by-zero issues for OFx, OFy, and OFz with explicit float `0.0`
        measurements[:, 15] = np.where(self.xsim[:, 4] != 0, self.xsim[:, 1] * self.xsim[:, 4], 0.0)  # OFx = Vx * Pmu
        measurements[:, 16] = np.where(self.xsim[:, 4] != 0, self.xsim[:, 3] * self.xsim[:, 4], 0.0)  # OFy = Vy * Pmu
        measurements[:, 17] = np.where(self.xsim[:, 4] != 0, self.xsim[:, 5] * self.xsim[:, 4], 0.0)  # OFz = Vz * Pmu
        ...
        Z = 1/self.xsim[:, 4]
        VZ = -self.xsim[:, 5]*Z**2
        measurements[:, 20] = VZ + self.xsim[:, 14]  # Az = Vz + Wz
        ...
        measurements[:, 23] = (np.cos(self.xsim[:, 6])*np.cos(self.xsim[:, 8]))*U1/self.params[0] - self.params[8]*(VZ + self.xsim[:, 14])/self.params[0] - 9.81
        ...
        if M is not None:
            ...
            measurement_map = {
                'Px': 0, 'Vx': 1, 'Py': 2, 'Vy': 3, 'Pmu': 4, 'Vmu': 5,
    ...
    def plot_states_targets_inputs(self):
        ...
        mpc_z=1/np.array(self.x_mpc.T[4])
        target_z=1/self.mu_target
        plt.figure()
        plt.plot(self.t_mpc,mpc_z,label='z',color='green',alpha=0.5,linewidth=w)
        plt.plot(self.t_mpc,target_z,label='z target',color='green',alpha=1,linestyle='dashed',linewidth=w)
        plt.xlabel('time')
        plt.ylabel('z_position')
        plt.legend()

        mpc_vz=-np.array(self.x_mpc.T[5])*mpc_z**2
        plt.figure()
        plt.plot(self.t_mpc,mpc_vz,label='vz',color='green',alpha=0.5,linewidth=w)
        plt.xlabel('time')
        ...
        plt.figure()
        plt.plot(self.t_mpc,np.array(self.x_mpc.T[4]),label='mu',color='green',alpha=0.5,linewidth=w)
        plt.plot(self.t_mpc,self.mu_target,label='mu target',color='green',alpha=1,linestyle='dashed',linewidth=w)
        plt.xlabel('time')
        plt.ylabel('mu_position')
        plt.legend()

        plt.figure()
        plt.plot(self.t_mpc,np.array(self.x_mpc.T[5]),label='vmu',color='green',alpha=0.5,linewidth=w)
        plt.xlabel('time')
        plt.ylabel('mu_velocity')
        plt.legend()
...
print('MPC_DRONE_WITH_WIND_PWM_Mu.py imported successfully!')