...
def ekf_mahalanobis(y, x0, f, h, Q, R, u, P0=None, threshold=None):
    '''
    ...
    for i in range(y.shape[1]):
        ...
        # Outlier rejection based on individual Mahalanobis distances
        R_mod = np.copy(R)
        for j in range(ny):
            # Calculate Mahalanobis distance for each measurement
            y_pred_j = h(xhat_fm[:, -1:], u[:, i:i+1])[j]
            S_j = C[j, :] @ P_fm[-1] @ C[j, :].T + R[j, j]
            mahalanobis_distance_j = ((y[j, i] - y_pred_j) ** 2) / S_j
            
            # Compare with corresponding threshold and update R_mod if necessary
            if mahalanobis_distance_j > threshold[j]:
                R_mod[j, j] = 1e9
                print(f"Iteration {i}, Measurement {j}: Mahalanobis distance caused a rejection.")
...
print("ekf_mahalanobis.py loaded successfully with Mahalanobis distance rejection")
