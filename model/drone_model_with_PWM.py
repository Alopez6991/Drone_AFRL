...
class DroneModel:
    ...
    def __init__(self):
        ...
        elif self.FLAG == '2' or self.FLAG == '3':
            self.state_names = ['x',  # x position in inertial frame [m]
                                'y',  # y position in inertial frame [m]
                                'z',  # z position in inertial frame [m]
                                'v_x',  # x velocity in body-level frame (parallel to heading) [m/s]
    ...
    def f3(self, X, U):
        ...
        # Velocity in body-level frame (adjusted for FLU)
        v_x_dot = (u_x- C * a_x/m) + v_y * psi_dot
        v_y_dot = (u_y- C * a_y/m) - v_x * psi_dot
        v_z_dot = (u_z- C * v_z/m)  - g
    ...
    def h2(self, X, U):
        ...
        # self.measurement_names = ['Ax','Ay','Az', 'rx','ry','Wax','Way']
        # Y = np.hstack((X, beta))
        Y = [x, y, z, v_x, v_y, v_z, psi, wx, wy,
             v_x_dot, v_y_dot, v_z_dot, rx, ry, a_x, a_y ]
    ...
    def h4(self, X, U):
        ...
        # States
        # print(np.shape(X))
        x, y, z, v_x, v_y, v_z, phi, theta, psi, wx, wy = X
        ...
        # Velocity in body-level frame (adjusted for FLU)
        v_x_dot = (1 / m) * (u_thrust * np.cos(phi) * np.sin(theta) - C * a_x) 
        v_y_dot = (1 / m) * (-u_thrust * np.sin(phi) - C * a_y)
        v_z_dot = (1 / m) * (u_thrust * np.cos(phi) * np.cos(theta) - C * v_z - m * g)
        ...
        # self.measurement_names = ['Ax','Ay','Az', 'rx','ry','Wax','Way']
        # Y = np.hstack((X, beta))
        Y = [x, y, z, v_x, v_y, v_z, phi, theta, psi, wx, wy,
             v_x_dot, v_y_dot, v_z_dot, rx, ry, a_x, a_y ]
    ...
    def h_c_car2(self, X, U, Y_SWEEP):
        ...
        V_cluster = np.array([Vx, Vy, Vz])
        Psi = Y[6]
        Attitude_cluster = np.array([Psi])
        Mocap_cluster = np.array([Px, Py, Pz, Vx, Vy, Vz, Psi])
        # IMU measurements
        Ax = Y[9]
        Ay = Y[10]
        Az = Y[11]
        A_cluster = np.array([Ax, Ay, Az])
        IMU_cluster = np.array([Psi,Ax, Ay, Az])
        # optical flow measurements
        OF_x = Y[12]
        OF_y = Y[13]
        OF_XY = np.array([OF_x, OF_y])
    ...
    def h_c_car4(self, X, U, Y_SWEEP):
        ...
        Az = Y[13]
        A_cluster = np.array([Ax, Ay, Az])
        IMU_cluster = np.array([Phi, Theta, Psi, Ax, Ay, Az])
        # optical flow measurements
        OF_x = Y[14]
        OF_y = Y[15]
        OF_XY = np.array([OF_x, OF_y])
    ...
    def z_function2(self, X):
        x, y, z, v_x, v_y, v_z, psi, wx, wy  = X
        g = (v_x ** 2 + v_y ** 2) ** (1 / 2)  # ground speed magnitude
        beta = sp.atan(v_y / v_x)  # ground speed angle
        W = (wx ** 2 + wy ** 2) ** (1 / 2)  # wind speed magnitude
        zeta = sp.atan(wy / wx)  # wind speed angle
        z = [x, y, z, g, beta, v_z, psi, W, zeta]
        return sp.Matrix(z)
    
    def z_function4(self, X):
        x, y, z, v_x, v_y, v_z, phi, theta, psi, wx, wy  = X
        g = (v_x ** 2 + v_y ** 2) ** (1 / 2)  # ground speed magnitude
        beta = sp.atan(v_y / v_x)  # ground speed angle
        W = (wx ** 2 + wy ** 2) ** (1 / 2)  # wind speed magnitude
        zeta = sp.atan(wy / wx)  # wind speed angle
        z = [x, y, z, g, beta, v_z, phi, theta, psi, W, zeta]
        return sp.Matrix(z)

    def simulate(self, x, u,DT):
        self.FLAG = drone_config.FLAG  # or 2
        ...
        for k in range(0, u.shape[0] - 1):
            ...
            if self.FLAG == '1':
                ...
                x_k = self.f_c_car3(x_sim[k], u_k, DT)
            elif self.FLAG == '4':
                x_k = self.f_c_car4(x_sim[k], u_k, DT)
            elif self.FLAG == '5':
                x_k = self.f_c_car5(x_sim[k], u_k, DT)
            # x_k = self.f_c_car(x_sim[k], u_k, DT)
            # x_k = self.f_c_car2(x_sim[k], u_k, DT)
...
class DroneSimulator(Simulator):
    def __init__(self,
        ...
        if self.FLAG == '1':
            ...
            print('in 3')
            super().__init__(self.dynamics.f3, self.dynamics.h3, dt=dt, mpc_horizon=mpc_horizon,
                            state_names=self.dynamics.state_names,
                            input_names=self.dynamics.input_names,
                            measurement_names=self.dynamics.measurement_names)
        elif self.FLAG == '4':
            print('in 4')
            super().__init__(self.dynamics.f4, self.dynamics.h4, dt=dt, mpc_horizon=mpc_horizon,
                            state_names=self.dynamics.state_names,
                            input_names=self.dynamics.input_names,
                            measurement_names=self.dynamics.measurement_names)
        elif self.FLAG == '5':
            print('in 5')
            super().__init__(self.dynamics.f5, self.dynamics.h5, dt=dt, mpc_horizon=mpc_horizon,
                            state_names=self.dynamics.state_names,
                            input_names=self.dynamics.input_names,
                            measurement_names=self.dynamics.measurement_names)
        ...
        # self.mpc.bounds['lower', '_u', 'u_z'] = 0
        ...
        if self.FLAG == '1':
            ...
            # for car
            self.mpc.set_rterm(u_x=r_u, u_y=r_u, u_z=r_u, u_psi=r_u)
        elif self.FLAG == '4' or self.FLAG == '5':
            # for car
